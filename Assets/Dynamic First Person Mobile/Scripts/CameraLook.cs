	...
	public class CameraLook : MonoBehaviour {	
		...
		private void Update() 
		{
			...
			foreach (var touch in Input.touches)
			{
				...
				{
					float xData = touch.deltaPosition.x;
					float yData = touch.deltaPosition.y;
					if (Mathf.Abs(touch.deltaPosition.x) > 2)
					{
						xData = Mathf.Sign(touch.deltaPosition.x) * 2;
					}
					if (Mathf.Abs(touch.deltaPosition.y) > 2)
					{
						yData = Mathf.Sign(touch.deltaPosition.y) * 2;
					}
					delta += new Vector2(xData, yData);
